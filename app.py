...
def predict():
    ...
    # --- CSV Writing ---
    try:
        csv_file_path = 'iit.csv'
        with open(csv_file_path, 'a', newline='') as csvfile:  # 'a' for append, newline='' to prevent extra rows
            writer = csv.writer(csvfile)
            if csvfile.tell() == 0:  # Write header only if the file is empty
                header = ['year', 'institute_t', 'round', 'no_quota', 'pool', 'institute_s',
                          'program_t', 'program_s', 'degree_s', 'st_category', 'opening_r',
                          'closing_r', 'r1_closing', 'r1_opening', 'prep_closing', 'prep_opening',
                          'predicted_college', 'predicted_degree', 'predicted_course']  # Added predicted columns
                writer.writerow(header)
            writer.writerow(list1 + list(output))  # Append prediction outputs to the row
    except Exception as e:
        print(f"Error writing to CSV: {e}")

    return render_template("home.html",
                           prediction_text="College : {} ,  Degree : {} , Course : {}".format(output[0], output[1],
                                                                                           output[2]),
